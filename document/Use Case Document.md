...
|  后置条件  | 存储更新客户的信息                                |
|  优先级   | 高                                        |
|  正常流程  | 1.系统生成并显示客户属性列表<br>2.销售人员进行增加客户、删除客户、修改客户属性或查询客户等处理<br>3.销售人员处理结束<br>4.系统更新并显示新的客户属性列表 |
|  扩展流程  | <br>2a.非法关键字<br>    1.系统提示错误并拒绝输入<br>2b.非法客户编号<br>    1.系统提示错误并拒绝输入<br>2c.销售人员修改应收额度<br>    1.系统查询系统用户的权限<br>    2.若不是最高权限，则提示没有最高权限拒绝修改<br>2d.销售人员增加已经存在的客户1.系统拒绝添加并提示客户已经存在 |
|  特殊需求  | 1.客户的属性有：编号、分类（进货商、销售商）、级别（五级，一级普通用户，五级VIP客户）、姓名、电话、地址、邮编、电子邮箱、应收额度、应收、应付、默认业务员2.查询客户可以通过输入关键字、客户编号等进行查找3.操作简单容易上手 |
...
|  后置条件  | 制定的进货单等待审批                               |
|  优先级   | 中                                        |
|  正常流程  | 1.销售人员新建一个进货单2.系统显示一个新进货单，包括：单据编号，供应商，仓库，操作员，入库商品列表，备注，总额合计。其中入库商品列表包含的信息有：商品编号，名称，型号，数量，单价，金额，备注3.系统自动计算并生成除名称、数量、备注等的其余部分4.销售人员手动输入、添加、选择入库商品列表中的名称、数量、备注等5.系统重新计算并生成显示进货单6.重复4~5步直至销售人员完成进货单7.系统生成最终进货单并显示 |
|  扩展流程  | 4a.销售人员更改单价1.系统更改进货单中的单价并显示4b.非法数量格式2.系统提示数量格式错误 |
|  特殊需求  | 1.单据编号格式为：JHD-yyyyMMdd-xxxxx，后五位每天从1开始编号，所以一天最多可以生成99999条单子2.名称从商品选择界面进行选择3.单价默认为商品信息中的进价4.进货单通过审批后，会更改库存数据和客户的应收应付数据5.操作简单容易上手 |
...
|  后置条件  | 制定的退货单等待审批                               |
|  优先级   | 高                                        |
|  正常流程  | 1.销售人员新建一个进货退货单2.系统显示一个新进货退货单，包括：单据编号，供应商，仓库，操作员，入库商品列表，备注，总额合计。其中入库商品列表包含的信息有：商品编号，名称，型号，数量，单价，金额，备注3.系统自动计算并显示除名称、数量、备注的其余部分4.销售人员在入库商品列表中的手动输入添加其名称、数量、备注等5.系统重新计算并生成进货退货单6.重复4~5步直到销售人员完成进货退货单7.系统生成并显示最终进货退货单 |
|  扩展流程  | 4a.销售人员更改入库商品列表的单价1.系统更改进货退货单中的单价并显示4b.非法数量格式1.系统提示数量格式错误 |
|  特殊需求  | 1.单据编号格式为：JHTHD-yyyyMMdd-xxxxx，后五位每天从1开始编号，所以一天最多可以生成99999条单子2.名称从商品选择界面进行选择3.单价默认为商品信息中的进价4.进货退货单通过审批后，会更改库存数据和客户的应收应付数据5.操作简单容易上手 |
...
|  后置条件  | 生成一个销售出货单，等待审批                           |
|  优先级   | 高                                        |
|  正常流程  | 1.销售人员新建一个销售出货单2.系统显示一个新销售出货单，包括：单据编号，客户，业务员，操作员，仓库，出货商品清单，折让前总额，折让，使用代金卷金额，折让后总额，备注。出货商品清单中要显示商品的编号，名称，型号，数量，单价，金额，商品备注。3.系统自动计算除名称、数量、备注、折让等的其余部分并显示4.销售人员手动输入入库商品列表中的名称、数量、备注等信息5.系统重新计算并显示销售出货单6.重复4~5步直至销售人员完成销售出货单7.系统生成最终的销售出货单并显示 |
|  扩展流程  | 4a.销售人员更改单价、业务员等默认选项1.系统更改销售单中的单价等默认选项并显示4b.销售人员设置折让1.系统查询系统使用者的权限，设置折让限额2.销售人员输入折让3.系统查询折让是否在限额内，若是则显示折让价格并重新计算金额，显示销售单4c.数量格式错误1.系统提示数量格式错误 |
|  特殊需求  | 1.单据编号的格式：XSD-yyyyMMdd-xxxxx，后五位每天从1开始编号，所以一天最多可以生成99999条单子2.客户：仅显示销售商3.业务员：和这个客户打交道的公司员工，可以设置一个客户的默认业务员4.操作员：当前登录系统的用户5.销售人员分为销售员和销售经理。销售员最多折让1000元，销售经理可以折让5000元，总经理可以任意金额的折让。6.名称：从商品选择界面选择7.单价：默认为商品信息里的销售价，可修改8.销售出货单通过审批后，会更改库存数据和客户的应收应付数据9.操作简单容易上手 |
...
|  后置条件  | 生成一个销售退货单，等待审核                           |
|  优先级   | 高                                        |
|  正常流程  | 1.销售人员新建一个销售退货单2.系统显示一个新销售退货单，包括：单据编号，客户，业务员，操作员，仓库，出货商品清单，折让前总额，折让，使用代金卷金额，折让后总额，备注。出货商品清单中要显示商品的编号，名称，型号，数量，单价，金额，商品备注3.系统自动计算除名称、数量、备注、折让等的其余部分4.销售人员手动输入、选择、添加入库商品列表中的名称、数量、备注等信息5.系统重新计算并生成显示销售退货单6.重复4~5步直至销售人员完成销售退货单7.系统生成最终销售退货单并显示 |
|  扩展流程  | 4a.销售人员更改单价、业务员等默认选项1.系统更改销售退货单中的单价等默认选项并显示4b.销售人员设置折让1.系统查询系统使用者的权限，设置折让限额2.销售人员输入折让3.系统查询折让是否在限额内，若是则显示折让价格并重新计算金额，显示销售退货单4c.数量格式错误1.系统提示数量格式错误 |
|  特殊需求  | 1.单据编号的格式：XSTHD-yyyyMMdd-xxxxx，后五位每天从1开始编号，所以一天最多可以生成99999条单子2.客户：仅显示销售商3.业务员：和这个客户打交道的公司员工，可以设置一个客户的默认业务员4.操作员：当前登录系统的用户5.销售人员分为销售员和销售经理。销售员最多折让1000元，销售经理可以折让5000元，总经理可以任意金额的折让6.名称：从商品选择界面选择7.单价：默认为商品信息里的销售价，可修改8.销售退货单通过审批后，会更改库存数据和客户的应收应付数据9.操作简单容易上手 |
...
|  扩展流程  | 2a. 表格填写不完全<br>    1.系统提示出错<br>    2.返回正常流程2<br>3a.单据审批通过<br>     1.系统更改客户的应付（收）数据<br>    2.系统给财务人员发送审批通过消息<br>3b.收款单审批未通过<br>    1.系统将未通过消息发送给财务人员 |
...
|  后置条件  | 减少相应账户余额                                 |
|  优先级   | 中                                        |
|  正常流程  | 1.系统生成并显示现金费用单，单据的信息包括单据编号，操作员（当前登录用户），银行账户，条目清单，总额。条目清单中包括：条目名，金额，备注。<br> 2.财务人员对现金费用单进行填写并进行提交，需要填写的部分为银行账户，条目清单及总额。<br>3. 系统将此单据入账并显示入账信息 |
|  扩展流程  | 2a. 表格填写不完全<br>    1.系统提示错误<br>    2.返回正常流程2<br>2b.所填账户余额不足<br>  1.系统提示财务人员余额不足<br>    2.返回正常流程2 |
...
|  后置条件  | 将该期初建账信息单独存储                             |
|  优先级   | 高                                        |
|  正常流程  | 1.系统生成期初建账的表格，表格的内容包括：添加商品信息（商品分类，某一商品的名称，类别，型号，进价/售价(默认为上年平均)，最近进价和最近售价留空），客户信息（客户分类，某一个客户的名称，联系方式等，应收应付(之前遗留)），银行账户信息（名称，余额）<br>2.财务人员填写表格并提交，需要填写的部分为添加商品信息，银行账户信息<br>3.系统显示建账结果 |
