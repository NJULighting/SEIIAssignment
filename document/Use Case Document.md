...
|  扩展流程  | 4.a 在根分类下请求进入父分类：<br> 系统提示当前分类没有父分类，返回正常流程步骤1 6.1a. 在已经存在商品的分类下添加子分类：<br>&emsp;系统提示分类不能添加在存在商品的分类下，返回正常流程第6步。<br> 6b. 删除已经存在商品/分类的分类：<br>&emsp;系统提示分类被删除的分类不能含有子分类或商品，返回正常流程第6步。<br> 6c. 添加已经存在的分类：<br>&emsp;  系统提示分类添加时不能添加当前目录下已经存在的分类，返回正常流程第6步。<br>6d. 修改后分类名和当前分类下的其他分类名称相同：<br>&emsp;系统提示修改后分类不能和当前分类下其他分类名称相同，返回正常流程第6步 |
|  特殊需求  | 简单易用，无操作难度。                              |
...
|  扩展流程  | 3a 财务人员点击取消<br>&emsp;1.系统退出期初建账任务        |
