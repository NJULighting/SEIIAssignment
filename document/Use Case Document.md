...
|  后置条件  | 如果分类信息保存成功，系统保存新商品分类管理信息，否则              |
|  优先级   | 中                                        |
|  正常流程  | 1 系统以树状结构显示当前的商品分类信息。<br> 2 库存管理人员根据需求对商品分类进行操作。<br> 3 保存最新的商品分类信息，系统提示成功。 |
|  扩展流程  | 2a. 在已经存在商品的分类下添加子分类：<br><t>   系统提示分类添加出错信息，返回正常流程第一步。<br> 2b. 删除已经存在商品/分类的分类：<br><t>   系统提示分类删除出错信息，返回正常流程第一步。<br> 2c. 添加已经存在的分类：<br><t>   系统提示分类添加出错信息，返回正常流程第一步。<br>2d. 修改后分类名和当前分类下的其他分类名称相同/不合法：<br><t> 系统知识分类名称冲突，返回正常流程第一步~~<br> 3a. 信息保存失败：<br><t>   系统提示保存失败，显示具体错误原因，返回正常流程第二步，即提交之前的分类状态。~~ |
|  特殊需求  | 简单易用，无操作难度。                              |
...
|  触发条件  | 库存管理人员对商品进行管理。                           |
|  前置条件  | 当前登录账号为库存管理人员账号，身份已验证无误。                 |
|  后置条件  | 如果行为是增删改且保存成功，系统保存最新新商品信息。否则，系统信息不发生变化。  |
|  优先级   | 高                                        |
...
|  扩展流程  | 2a. 添加已有的商品（根据商品名称、型号来进行确定是否相同）/ 在有分类的目录下：<br><t>   系统提示商品添加出错信息，返回正常流程第一步。<br> 2b. 修改商品信息时出现不合法内容/修改后商品信息和其他商品信息冲突：<br><t>   系统提示商品信息修改错误原因，返回正常流程第一步。2c.删除库存不为0或存在交易记录的商品：<br><t> 系统提示删除出错，返回正常流程第一步~~<br> 3a. 信息保存失败：<br><t>   系统提示保存失败，显示具体错误原因，返回正常流程第二步，即提交之前的商品状态。~~ |
|  特殊需求  | 简单易用，无操作难度。                              |
...
|  优先级   | 中                                        |
|  正常流程  | 1 库存管理人员选择的商品。<br> 2 输入当前产品的实际数量。<br> 3 输入备注信息<br> 4 提交单据。 |
|  扩展流程  | 2a. 商品数量不合法（如输入小于零的数）：<br><t>   系统提示数量不合法，返回正常流程第一步。<br> 2b. 输入非法字符：<br><t>   系统提示输入不合法，返回正常流程第一步。2c. 取消单据：单据删除，系统结束流程~~<br> 3a. 单据提交失败：<br><t>   系统提示单据提交失败，显示具体错误原因，返回正常流程第一步。~~ |
|  特殊需求  | 简单易用，无操作难度。                              |
...
|  触发条件  | 库存管理人员创建库存报警单。                           |
|  前置条件  | 当前登录账号为库存管理人员账号，身份已验证无误。                 |
|  后置条件  | 如果成功提交单据，报警单存储，等待触发。否则                   |
|  优先级   | 中                                        |
|  正常流程  | 1 库存管理人员选择商品，警戒数量。<br> 2 提交单据。           |
|  扩展流程  | 1a. 商品数量不合法：<br><t>   系统提示数量不合法，显示当前该类商品数量，返回正常流程第一步。<br>  1b. 输入非法字符：<br><t>   系统提示输入不合法，返回正常流程第一步。 1c. 取消单据： <br><t> 系统结束流程~~<br> 2a. 单据提交失败：<br><t>   系统提示单据提交失败，显示具体错误原因，返回正常流程第一步。~~ |
|  特殊需求  | 简单易用，无操作难度。                              |
...
|  触发条件  | 库存管理人员处理信息。                              |
|  前置条件  | 当前登录账号为库存管理人员账号，身份已验证无误。                 |
|  后置条件  | 如果消息被处理，从未处理的消息队列中移除，否则未读消息保持不变          |
|  优先级   | 高                                        |
