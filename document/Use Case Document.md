...
|  后置条件  | 新的商品分类被保存。                               |
|  优先级   | 中                                        |
|  正常流程  | 1 系统显示当前的商品及商品分类信息。<br>2.1<br>&emsp;2.1.1 库存管理人员请求添加商品分类，选择添加位置，输入新的商品分类名称<br> 2.1.2 系统保存新的商品分类，显示最新的商品及商品分类信息。<br>2.2<br>&emsp; 2.2.1 库存管理人员请求修改商品分类，选择要修改的分类，输入新的分类名称<br> 2.2.2 系统保存修改后的的商品分类，显示最新的商品及商品分类信息<br>2.3<br>&emsp; 2.3.1 库存管理人员请求删除分类，选中分类，进行删除操作<br>&emsp; 2.3.2 系统删除分类，显示最新的商品及商品分类信息 3 重复上述步骤2，直到所有的商品分类操作完成 |
|  扩展流程  | 2.1a. 在已经存在商品的分类下添加子分类：<br>&emsp;系统提示分类不能添加在存在商品的分类下，返回正常流程第2步。<br> 2b. 删除已经存在商品/分类的分类：<br>&emsp;系统提示分类被删除的分类不能含有子分类或商品，返回正常流程第2步。<br> 2c. 添加已经存在的分类：<br>&emsp;  系统提示分类添加时不能添加当前目录下已经存在的分类，返回正常流程第2步。<br>2d. 修改后分类名和当前分类下的其他分类名称相同：<br>&emsp;系统提示修改后分类不能和当前分类下其他分类名称相同，返回正常流程第一步 |
|  特殊需求  | 简单易用，无操作难度。                              |
...
|  后置条件  | 新的商品分类被保存。                               |
|  优先级   | 高                                        |
|  正常流程  | 1 系统显示商品及商品分类信息，包括各个分类下商品的属性，包括编号、名称、型号、库存数量、进价、零售价、最近进价、最近零售价。<br>2.1<br>&emsp; 2.1.1 库存管理人员选择添加商品， 输入新商品的名称、型号 <br>&emsp;2.1.2 系统自动生成编号，用0填充库存数量、进价、零售价、最近进价、最近零售价，保存新添加商品的信息，显示添加商品后的商品列表 <br>2.2<br>&emsp;2.2.1 库存管理人员选择修改商品，选中要修改的商品，在要修改的属性处输入新的属性<br>&emsp; 2.2.2 系统保存修改后的信息，显示新的商品信息列表  <br>2.3<br>&emsp;2.3.1 库存管理人员选择删除商品，选中商品后删除 <br>&emsp;2.3.2 系统删除商品信息，显示新的商品信息列表<br>2.4<br>&emsp; 2.4.1 库存管理人员选择查询商品，输入商品的编号或者商品的名称，进行查询 <br>&emsp;2.4.2 如果存在匹配的商品，系统显示匹配的商品条目，否则显示空 3 重复上述步骤2，直到所有的商品管理操作完成 |
|  扩展流程  | 2.1a. 添加已有的商品（根据商品名称、型号来进行确定是否相同）/ 在有分类的目录下：<br>&emsp;  系统提示商品添加必须和其他商品无冲突并且只能在没有分类的目录下添加并提示错误原因（如和哪些商品冲突），返回正常流程第2步。<br> 2.2b. 修改商品信息时出现不合法内容/修改后商品信息和其他商品信息冲突：<br>&emsp;系统提示商品信息修改错误原因（如和哪些商品冲突），返回正常流程第2步。2.3c.删除库存不为0或存在交易记录的商品：<br>&emsp;系统提示删除不能删除存在交易记录或者库存不为0的商品，返回正常流程第2步 |
|  特殊需求  | 简单易用，无操作难度。                              |
