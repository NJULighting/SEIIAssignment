...
- 提供的服务
  ...
  | 名称                                  | 条目   | 详情                                       |
  | ----------------------------------- | ---- | ---------------------------------------- |
  |                                     | 语法   | `public String getAccountInOutInfo()`    |
  | AccountInOutDoc.getAccountInOutInfo | 前置条件 | 得到数据                                     |
  |                                     | 后置条件 | 返回根据所得数据生成的单据编号以及当前操作员的字符串               |
...
- 需要的服务（需接口）
  ...
  | UserDataService.insert(AccountPO po) | 插入单一持久化对象          |
  | UserDataService.update(AccountPO po) | 更新单一持久化对象          |
  | UserDataService.delete(AccountPO po) | 删除单一持久化对象          |
