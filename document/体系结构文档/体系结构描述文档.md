...
| 廖均达  | 2017.12.2  | 修改UserBLService接口                        | 1.0.5 |
...
|                             | 语法   | `public ResultMessage changePassword(String id, String newPassword)` |
| User.administchangePassword | 前置条件 | 传入不为空的password                           |
|                             | 后置条件 | 更改用户的password字段信息，更新持久化存储的数据             |
|                             | 语法   | `public ResultMessage changeName(String id, String newName)` |
| User.changeName             | 前置条件 | 用户请求修改用户名且传入的用户名不含除数字、字母、中文以外的字符         |
|                             | 后置条件 | 更改用户的name字段，更新持久化存储数据                    |
