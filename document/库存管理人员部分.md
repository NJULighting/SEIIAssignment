...
- 提供的服务
  ...
  |                                  | 语法   | `public ArrayList<HistoryDoc> getHistoryDocs(int useid, int startCount, int endCount)` |
  | HistroyDoc.getHistoryDocs        | 前置条件 | 输入有效的用户ID，以及倒序起点startCount和倒序endCount    |
...
| DocDataService.findByUser(int userid)    | 获取某一用户的全部单据信息 |
