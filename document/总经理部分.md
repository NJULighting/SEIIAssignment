...
| 服务名                               | 服务                 |
| --------------------------------- | ------------------ |
| ApprovalblService.getDocumentList | 返回所有处于待审批状态的单据的值对象 |
| ApprovalblService.approve         | 通过正在查看的单据          |
| ApprovalblService.approveAll      | 通过所选的所有单据          |
| ApprovalblService.reject          | 驳回正在查看的单据          |
| ApprovalblService.save            | 保存修改好的单据           |
| ApprovalblService.saveAndApprove  | 保存修改好的单据并同时通过审批    |
...
| 名称                       | 条目   | 详情                                       |
| ------------------------ | ---- | ---------------------------------------- |
|                          | 语法   | `public ArrayList<HitoryDocVO> getDocumentList()` |
| Approval.getDocumentList | 前置条件 | 用户已登录                                    |
|                          | 后置条件 | 返回含有所有处于待审批状态的单据的列表的值对象                  |
|                          | 语法   | `public ResultMessege approve(HistoryDocVO vo)` |
| Approval.approve         | 前置条件 | 用户有正在查看的单据                               |
|                          | 后置条件 | 通过单据，持久化更新涉及的对象的数据                       |
|                          | 语法   | `public ResultMessege approveAll(ArrayList<HistoryDocVO> voList)` |
| Approval.approveAll      | 前置条件 | 用户有选择的要通过的单据                             |
|                          | 后置条件 | 通过所选的单据，持久化更新涉及的对象的数据                    |
|                          | 语法   | public ResultMessege reject(HistoryDocVO vo) |
| Approval.reject          | 前置条件 | 用户有正在查看的单据                               |
|                          | 后置条件 | 驳回单据，持久化更新涉及的对象的数据                       |
|                          | 语法   | `public ResultMessege save(HistoryDocVO vo)` |
| Approval.save            | 前置条件 | 有已修改要保存的单据                               |
|                          | 后置条件 | 持久化更新涉及的对象的数据                            |
|                          | 语法   | `public ResultMessege saveAndApprove(HistoryDocVO vo)` |
...
|                          | 语法   | `public ResultMessege update(HistoryDocPO po)` |
...
| 服务名                         | 服务           |
| --------------------------- | ------------ |
| Promortion.getPromotionList | 得到所有促销策略的值对象 |
| Promotion.newAndSave        | 保存新建立的促销策略   |
| Promotion.modifyAndSave     | 更新修改后的促销策略   |
| Promotion.delete            | 删除正在查看的促销策略  |
...
|                            | 后置条件 | 返回含有所有促销策略的值V对象的列表                       |
|                            | 语法   | `public ResultMessege newAndave(PromotionVO vo)` |
| Promotion.newAndSave       | 前置条件 | 用户有需要保存的新建的销售策略                          |
|                            | 后置条件 | 新建单据，持久化更新相关的对象的数据                       |
|                            | 语法   | `public ResultMessege modifyAndSave(PromotionVO vo)` |
| Promotion.modifyAndSave    | 前置条件 | 用户有需要保存的修改好的单据                           |
|                            | 后置条件 | 持久化更新相关的对象的数据                            |
|                            | 语法   | `public ResultMessege delete(PromotionVO vo)` |
...
|                                       | 语法   | `public ResultMessege insert(PromotionPO po)` |
| PromotionDataService.insert           | 前置条件 | 同样ID的po在数据库中不存在                          |
|                                       | 后置条件 | 在数据库中增加一个po记录                            |
|                                       | 语法   | `public ResultMessege update(PromotionPO po)` |
| PromotionDataService.update           | 前置条件 | 在数据库中存在同样ID的po                           |
|                                       | 后置条件 | 更新一个po                                   |
|                                       | 语法   | `public ResultMessege delete(PromotionPO po)` |
