...
| 2018.1.12  | 廖均达  | 修改docbl、approvalbl模块 | 1.1  |
| 2018.1.12  | 訾源   | 修改commoditybl        | 1.2  |
| 2018.1.12  | 高梦婷  | 修改customerbl         | 1.3  |
| 2018.1.12  | 訾源   | 数据层的修改               | 1.4  |
| 2018.1.12  | 訾源   | 添加参考文献               |      |
...
![promotionbl模块](Image/设计类图/promotionbl.png)
...
Approval模块的所有功能均由docbl模块提供的接口实现，approval模块只需要创建DocInfo的实现类，然后调用相应的方法来完成相应的功能

![approval_idea](C:\Users\liao\Documents\SEIIAssignment\document\详细设计文档\Image\设计类图\approval_idea.png)
...
根据体系结构的设计，我们将系统分为展示层、业务逻辑层、数据层。每一层之间为了增加灵活性，我们会添加接口。比如展示层和业务逻辑层之间，我们添加bussinesslogicservice.docblservice.DocBLService接口。业务逻辑层和数据层之间添加dataservice. DocDataService接口。为了隔离业务逻辑职责和逻辑控制职责，我们增加了DocController，这样DocController会将单据的逻辑业务处理委托给DocManager、RedFlush、SalesDetailTable类，DocManager类负责实现单据的提交、查找功能，RedFlush类负责实现单据的红冲、红冲并复制功能，SalesDetailTable负责实现查看经营情况表的功能，Doc类时单据的领域模型，是各种具体单据类的父类（抽象类），具体的单据的属性与方法由具体的子类提供，每个单据有与之对应的Item类以及列表类，Item是单据中的小项的领域模型，列表类是用存放Item的容器类，所有Doc的Item类需要继承DocItem接口，所有的列表类通过组合Itemlist工具类来实现。DocInfo是docbl模块对其他模块开放的接口，其他模块通过调用DocInfo提供的方法来获得docbl模块的信息或者调用docbl的方法

docbl各个类的职责如表所示

| 类                | 职责                                       |
| ---------------- | ---------------------------------------- |
| DocManager       | 负责实现单据的提交、查找单据以及丢弃报警单的功能                 |
| RedFlush         | 负责实现单据的红冲功能                              |
| SalesDetailTable | 负责实现计算销售明细表的各项数据，并生成销售明细表的值对象            |
| DocInfo          | 负责为其他需要与docbl模块交互的模块提供接口                 |
| DocController    | 负责实现DocBLService的方法，将各个方法的具体实现委托给DocManager、RedFlush和SalesDetailTable类 |
| Doc              | 单据的领域模型对象，含有所有单据共有的数据，提供转换成各种值对象的方法，红冲时修改相应属性的方法，审批时修改相应数据 |
| DocItem          | 单据小项的领域模型需要实现的接口，配合ItemList类完成单据小项的红冲，审批，修改id的方法 |
| ItemList类        | 单据列表的工具类，通过泛型实现，主要功能为对列表中的Item进行管理，统一调用DocItem接口中的approve，redflush方法以辅助实现整个单据的审批、红冲功能 |
| CostDoc          | 现金费用单的领域模型对象，是Doc类的子类，实现了相应的抽象方法         |
| AccountInDoc     | 收款单的领域模型对象，是Doc类的子类，实现了相应的抽象方法           |
| AccountOutDoc    | 付款单的领域模型对象，是Doc类的子类，实现了相应的抽象方法           |
| AlertDoc         | 库存报警单的领域模型对象，是Doc类的子类，实现了相应的抽象方法         |
| StockReturnDoc   | 进货退货单的领域模型对象，是Doc类的子类，实现了相应的抽象方法         |
| GiftDoc          | 赠品单的领域模型对象，是Doc类的子类，实现了相应的抽象方法           |
| LossAndGainDoc   | 报损报溢单的领域模型对象，是Doc类的子类，实现了相应的抽象方法         |
| SalesDoc         | 销售单的领域模型对象，是Doc类的子类，实现了相应的抽象方法           |
| StockDoc         | 进货单的领域模型对象，是Doc类的子类，实现了相应的抽象方法           |
| SalesReturnDoc   | 销售退货单的领域模型对象，是Doc类的子类，实现了相应的抽象方法         |
...
| 名称           | 条目   | 详情                                       |
| ------------ | ---- | ---------------------------------------- |
|              | 语法   | `abstract protected ResultMessage approve();` |
| Doc.approve  | 前置条件 | 已经创建一个Doc领域对象                            |
|              | 后置条件 | 修改该单据审批通过后应该修改的数据以及状态，并更新持久化存储           |
|              | 语法   | `abstract protected void redFlush();`    |
| Doc.redFlush | 前置条件 | 已经创建一个Doc领域对象                            |
|              | 后置条件 | 修改该单据进行红冲时应该修改的数据以及状态                    |
|              | 语法   | `abstract public DocPO toPO();`          |
| Doc.toPO     | 前置条件 | 已经创建一个Doc领域对象                            |
|              | 后置条件 | 生成相应的PO对象                                |
|              | 语法   | `abstract public DocVO toVO();`          |
| Doc.toVO     | 前置条件 | 已经创建一个Doc领域对象                            |
|              | 后置条件 | 生成相应的VO对象                                |
...
| 名称                                      | 条目   | 详情                                       |
| --------------------------------------- | ---- | ---------------------------------------- |
|                                         | 语法   | `public List<SaleRecordItemVO> findSaleRecords(DocumentFilter filter)` |
| DocController.findSaleRecords           | 前置条件 | 已经创建一个DocManager领域对象，已添加筛选条件             |
|                                         | 后置条件 | 调用DocManager对象的findSaleRecords方法         |
|                                         | 语法   | `public List<BussinessHitoryItemVO> findDocuments(DocumentFilter filter)` |
| DocController.findDocuments             | 前置条件 | 已经创建一个Doc领域对象，已添加筛选条件                    |
|                                         | 后置条件 | 调用DocManager对象的的findDocuments方法          |
|                                         | 语法   | `public BusinessConditionTableVO findRevenueAndExpenditure(Date startDate, Date endDate)` |
| DocController.findRevenueAndExpenditure | 前置条件 | 已经创建一个DocManager对象，已添加筛选条件               |
|                                         | 后置条件 | 调用SalesDetailTable对象的getSalesDetailTable方法 |
|                                         | 语法   | `public List<BussinessHitoryItemVO> findBusinessHistory(DocumentFilter filter)` |
| DocController.findBusinessHistory       | 前置条件 | 已经创建一个DocManager对象                       |
|                                         | 后置条件 | 调用DocManager对象的findBusinessHistory方法     |
|                                         | 语法   | `public ResultMessage redflush(DocVO target)  ` |
| DocController.redflush                  | 前置条件 | 已经创建一个RedFlush领域对象，传入有效的DocVO对象          |
|                                         | 后置条件 | 调用RedFlush领域对象的redflush方法                |
|                                         | 语法   | `public Result<DocVO> commitDoc(DocVO doc)  ` |
| DocController.commitDoc                 | 前置条件 | 已经创建一个DocManager领域对象，传入对象不为null，且包含信息正确有效 |
|                                         | 后置条件 | 调用DocManager对象的commitDoc方法               |
|                                         | 语法   | `ResultMessage expireAlertDoc(AlertDocVO alertDocVO);` |
| DocController.expireAlertDoc            | 前置条件 | 已经创建一个DocManager领域对象，传入对象不为null，且包含信息正确有效 |
|                                         | 后置条件 | 调用DocManager对象的expireAlertDoc方法          |
...
| ------------------------------ | ---- | ---------------------------------------- |
|                                | 语法   | `public Result<DocVO> commitDoc(DocVO doc)  ` |
| DocManager.commitDoc           | 前置条件 | 传入对象不为null，且包含信息正确有效                     |
|                                | 后置条件 | 将DocVO提交至数据库                             |
|                                | 语法   | `public List<HistoryDocVO> findDocuments(DocumentFilter filter) ` |
| DocManager.findDocuments       | 前置条件 | 传入一个有效的DocumentFilter                    |
|                                | 后置条件 | 返回符合DocumentFilter所设定条件的所有Doc            |
|                                | 语法   | `public List<SalesDetailItemVO> findSaleRecords(DocumetFilter filter)` |
| DocManager.findSaleRecords     | 前置条件 | 传入一个有效的DocumentFilter                    |
|                                | 后置条件 | 返回符合筛选条件的所有销售记录的值对象                      |
|                                | 语法   | `public List<BussinessHitoryItemVO> findDocuments(DocumentFilter filter)` |
| DocManager.findBusinessHistory | 前置条件 | 传入一个有效的DocumentFilter                    |
|                                | 后置条件 | 返回符合筛选条件的所有单据的值对象                        |
|                                | 语法   | `ResultMessage expireAlertDoc(AlertDocVO alertDocVO);` |
| DocController.expireAlertDoc   | 前置条件 | 传入有效的AlertDocVO对象                        |
|                                | 后置条件 | 对相应的报警单进行废弃操作                            |
...
| ----------------- | ---- | ---------------------------------------- |
|                   | 语法   | `public ResultMessage redflush(DocVO target)  ` |
| RedFlush.redflush | 前置条件 | 传入有效的DocVO对象                             |
|                   | 后置条件 | 将对应红冲单据入账，更新持久化存储，更新显示                   |


###### SalesDetailTable的接口规范

| 名称                                       | 条目   | 详情                                       |
| ---------------------------------------- | ---- | ---------------------------------------- |
|                                          | 语法   | `public RevenueAndExpenditureVO getSalesDetailTable()` |
| SalesDetailTable.findRevenueAndExpenditure | 前置条件 | 用户已输入正确的筛选条件并确认                          |
|                                          | 后置条件 | 返回所输入时间段内的收支情况的值对象                       |

###### <span id="di">DocInfo的接口规范</span>
| 名称                       | 条目   | 详情                                       |
| ------------------------ | ---- | ---------------------------------------- |
|                          | 语法   | `List<DocVO> findUnCheckedDoc();`        |
| DocInfo.findUnCheckedDoc | 前置条件 | 用户已经创建一个DocInfo的实现类的对象                   |
|                          | 后置条件 | 返回一个包含所有未经过审批的单据的列表                      |
|                          | 语法   | `ResultMessage approve(HistoryDocVO vo);` |
| DocInfo.approve          | 前置条件 | 用户已经创建一个DocInfo的实现类的对象                   |
|                          | 后置条件 | 完成审批单据后相应的数据修改、状态修改的操作，更新持久数据            |
|                          | 语法   | `ResultMessage approveAll(List<HistoryDocVO> voList);` |
| DocInfo.approveAll       | 前置条件 | 用户已经创建一个DocInfo的实现类的对象                   |
|                          | 后置条件 | 完成审批整个列表的单据后相应的数据修改、状态修改的操作，更新持久数据       |
|                          | 语法   | `ResultMessage reject(HistoryDocVO vo);` |
| DocInfo.reject           | 前置条件 | 用户已经创建一个DocInfo的实现类的对象                   |
|                          | 后置条件 | 修改单据的状态为被拒绝，更新持久化数据，同时调用相关方法发送消息给单据的创建者  |
|                          | 语法   | `ResultMessage save(HistoryDocVO vo);`   |
| DocInfo.save             | 前置条件 | 用户已经创建一个DocInfo的实现类的对象                   |
|                          | 后置条件 | 更新持久化存储                                  |
|                          | 语法   | `void triggerAlertDoc(String commodityId, int count);` |
| DocInfo.triggerAlert     | 前置条件 | 用户已经创建一个DocInfo的实现类的对象                   |
|                          | 后置条件 | 修改符合触发条件的报警单的触发状态，更新持久化存储                |
...
| Logger.add                               | 提供日志记录接口    |
| UserInfo.getIDOfSignedUser               | 得到创建者信息     |
| UserInfo.getUserVOByID                   | 得到相关用户的值对象  |
| AccountInfo.addAmount                    | 修改账户的金额     |
| AccountInfo.getAccountById               | 得到账户的VO对象   |
| CommodityInfo.getBasicCommodityItemVO    | 得到商品的VO对象   |
| CommodityInfo.getCommodityRecentSellPrice | 得到某商品的最近售价  |
| CommodityInfo.getCommodityNameByID       | 得到某商品的名称    |
| CommodityInfo.getCommodityInPrice        | 得到某商品的进价    |
| CommodityInfo.getCostAdjustRevenue       | 得到商品调价得到的收入 |
| CustomerInfo.changeReceivable            | 增加客户应收      |
| CustomerInfo.changePayable               | 增加客户应付      |
| RepositoryInfo.changeRepository          | 改变库存        |
...
数据层中巨大多数包只是实现了体系结构设计文档的接口，详情参见数据层的分解文件。
