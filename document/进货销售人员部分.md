...
| 名称                               | 条目   | 详情                                       |
| -------------------------------- | ---- | ---------------------------------------- |
|                                  | 语法   | `public boolean addStockDoc(StockDocVO stockDoc) throws RemoteException` |
| StockDoc.addStockDoc             | 前置条件 | 已在新建进货单进程中，且已经输入有效的进货单信息：单据编号，供应商，仓库，操作员，入库商品列表，备注，总额合计 |
|                                  | 后置条件 | 新建进货单，增加库存，并更新持久化数据                      |
|                                  | 语法   | `public boolean addStockReturnDoc(StockReturnDocVO stockReturnDoc) throws RemoteException` |
...
|                                  | 语法   | `public boolean addSalesDoc(SalesDocVO salesDoc) throws RemoteException` |
...
|                                  | 语法   | `public boolean addSaleReturnDoc(SalesReturnDocVO salesReturnDoc) throws RemoteException` |
