...
CommodityListItem表：（对应CommodityListItemPO表）

| 名称                | PO对象数据类型 | hibernate数据类型 | 备注          | 含义                                       |
| ----------------- | -------- | ------------- | ----------- | ---------------------------------------- |
| id                | int      | int           | primary key | 商品清单列表条目id，不对外显示                         |
| SalesOrStockDocId | String   | String        |             | 条目所属的单据编号id(进货单、进货退货单、销售单、销售退货单)，与所在单据所对应的（StockPO,StockReturnPO,SalesPO,SalesReturnPO)）中的id一致（可能有多条条目对应同一个单据，一个单据里的多条条目组合起来成为一个“商品列表”） |
...
| salePrice         | double   | double        |             | 此条目中商品的售价，对应商品项CommoItem中recentSalePrice |
